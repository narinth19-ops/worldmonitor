...
import { buildMacroEventReactions, persistReactionSample, readReactionSamples } from '../src/services/macro-event-reactions.ts';
...
describe('macro event reactions', () => {
  it('calculates observable market changes from the nearest pre/post samples', () => {
    ...
    assert.equal(reactions[0]?.elapsedMinutes, 10);
